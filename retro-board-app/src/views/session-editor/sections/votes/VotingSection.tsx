...
function VotingSection({ options, onChange }: VotingSectionProps) {
  ...
  const handleChange = useCallback((prop: keyof SessionOptions) => function<T>(value: T) {
    onChange({
      ...options,
      [prop]: value,
    });
  ...
  const setAllowSelfVoting = useCallback(handleChange('allowSelfVoting'), [handleChange]);
  const setAllowMultipleVotes = useCallback(handleChange('allowMultipleVotes'), [handleChange]);
  const setMaxUpVotes = useCallback(handleChange('maxUpVotes'), [handleChange]);
  const setMaxDownVotes = useCallback(handleChange('maxDownVotes'), [handleChange]);
  return (
    <SettingCategory
  ...
  >
    ...
    >
      <MaxVoteSlider value={options.maxDownVotes} onChange={setMaxDownVotes} />
    </OptionItem>
    <OptionItem
